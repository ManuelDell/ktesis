        ...
        <table class="min-w-[700px] w-full text-xs">
          ...
          <tbody>
            ...
            <tr class="border-t-2 border-outline-gray-3 bg-surface-gray-1 font-semibold">
              <td class="py-2 px-3 text-ink-gray-8 sticky left-0 bg-surface-gray-1">Saldo</td>
              <td
                v-for="m in jahresData"
                :key="m.monat"
                class="py-2 px-3 text-right font-semibold"
                :class="m.kategorien.reduce((s,k)=>s+(k.ist||0),0) >= 0 ? 'text-ink-green-3' : 'text-ink-red-4'"
              >
                {{ formatCurrency(m.kategorien.reduce((s,k)=>s+(k.ist||0),0)) }}
              </td>
              <td class="py-2 px-3 text-right font-bold"
                :class="jahresData.reduce((sum,m)=>sum+m.kategorien.reduce((s,k)=>s+(k.ist||0),0),0) >= 0 ? 'text-ink-green-3' : 'text-ink-red-4'">
