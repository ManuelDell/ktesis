  ...
  <div class="p-5">
    ...
    <template v-if="!selectedName">
      ...
      <div class="flex items-center justify-between gap-4 mb-6">
        <h2 class="text-3xl font-semibold text-ink-gray-9 truncate">Darlehen</h2>
          ...
          <div class="flex items-start justify-between">
            <div @click="openDetail(item.name)" class="flex-1 min-w-0">
              ...
              <div class="flex gap-5 mt-2 text-sm">
                <div>
                  <span class="text-gray-500">Darlehensbetrag:</span>
                  <span class="ml-1 font-medium">{{ formatCurrency(item.darlehensbetrag) }}</span>
                </div>
                <div>
                  <span class="text-gray-500">Restschuld:</span>
                  <span class="ml-1 font-medium">{{ formatCurrency(item.restschuld) }}</span>
                </div>
              </div>
            </div>
    ...
    <!-- Create Dialog -->
    <Dialog
      :options="{ title: 'Darlehen anlegen', size: 'xl' }"
    ...
    <!-- Delete Dialog -->
    <Dialog
      :options="{ title: 'Darlehen löschen', size: 'sm', actions: [
...
const filteredList = computed(() => {
  return listData.value.filter((item) => {
    if (search.value) {
      ...
      if (!bezeichnung.includes(s) && !darlehensgeber.includes(s)) {
        return false
      }
    }
    return true
...
async function doDelete() {
  ...
  try {
    await delete_('Darlehen', itemToDelete.value.name)
