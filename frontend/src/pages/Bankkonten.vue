  ...
  <div class="kt-screen kt-screen-enter">
    <template v-if="!selectedName">
      <div class="kt-screen-header">
        <div>
          <h1 class="kt-hub-greeting">Bankkonten</h1>
          <p class="kt-hub-sub">Verwaltung der Bankkonten und Salden</p>
        </div>
      ...
      <div v-if="loading" class="kt-list-empty" style="padding:60px 22px">
        <p style="margin:0;font-size:13px;color:var(--kt-text-muted)">Lade Bankkonten…</p>
      </div>

      <div v-else-if="filteredList.length === 0" class="kt-list-empty" style="padding:60px 22px">
        <p style="margin:0;font-size:13px;color:var(--kt-text-muted)">Keine Bankkonten gefunden.</p>
      </div>
      ...
      <div v-else class="kt-list">
        ...
        >
          <div class="kt-list-item-body">
            ...
            <div class="kt-list-item-sub">{{ item.bank || '—' }} · {{ item.kontotyp || '—' }}</div>
            <div class="kt-list-item-meta">
              <span>Kontostand: <strong>{{ formatCurrency(item.kontostand) }}</strong></span>
            </div>
          </div>
        </div>
      </div>
...
const filteredList = computed(() => {
  return listData.value.filter((item) => {
    if (search.value) {
      ...
      if (!bezeichnung.includes(s) && !bank.includes(s)) return false
    }
    return true
  })
})
